...
test_key= 0xF0C6
input_d= 0x0b00
e_output_d= 0x66      # s-box 2 and 4, from msb
...
for entry in range(10000): 
    ...
    for out in range(2^8):
        ...
        for i in range(len(out_key)): out_b+= [out_key[i]]+ [0]
        ...
        cha= y_diff[0:4]+y_diff[8:12]
        if frombits(cha)== e_output_d: y[out]+=1

print(hex(y.index(max(y))), (max(y)/10000).n())
print(hex(y.index(max(y))^^2))
print(time()-start_time)
